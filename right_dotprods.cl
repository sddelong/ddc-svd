...
/**********************************************************
 * kernel: right_dotprods
 * 
 * Original Author: Travis Askham (12/20/2012)
 * 
 * Description: This kernel is used to compute the inner products of 
 * the given partial row (reflector) with the rows below it. 
 * It writes the resulting inner products to the scratch vector at 
 * position i where the inner product is taken with the partial row 
 * i+1 to the below the reflector. 
 * 
 * Input: as above, and
 * 
 * 	mat - the matrix whose rows are multiplied
